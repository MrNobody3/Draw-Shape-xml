...
public class PaintPanel extends JPanel{
			...
			public void mouseReleased(MouseEvent e) {
				...
				if(drawMode){
					...
					if (value.equals("Line")){
						...
						figure.getRectangles().add(new Rectangle(pointA,pointB));
					}
					else{
	...
	public void drawRectangles(Graphics g){
		for(Rectangle it : figure.getRectangles()){
			//int hl=(int) Math.sqrt(Math.pow(it.getPa().getX()-it.getPb().getX(), 2)+Math.pow(it.getPa().getY()-it.getPb().getY(), 2));
			 g.drawRect(it.getPa().getX(), it.getPa().getY(),it.getPb().getX()-it.getPa().getX(),it.getPb().getY()-it.getPa().getY());
		}
	}
	@Override
